...
def SSC_test():
    ...
    X1 = np.random.randn(D, d1) * np.random.randn(d1, N1) + noise * np.random.randn(D, N1) + affine * np.random.randn(D, 1)
    # Generating N2 points in a d2 dim. subspace
    X2 = np.random.randn(D, d2) * np.random.randn(d2, N2) + noise * np.random.randn(D, N2) + affine * np.random.randn(D, 1)
